...
class IntervieweeController extends RestController
{
    public $modelClass = 'api\modules\d1\models\Interviewee';
    ...
    public function init()
    {
        ...
        if(class_exists('api\modules\d1\models\Interviewee'))
            $this->modelClass = 'api\modules\d1\models\Interviewee';
        else
    ...
    public function actions()
    {
		...
		$actions['index'] = [
			...
			'prepareDataProvider' => function () {
                ...
                $query = $model::find(); //->where(['published'=>0]);
                $query->joinWith('interviews');
                $query->where('interview.published=1');

                $filter = new ActiveDataFilter([
                    'searchModel' => 'api\modules\d1\models\IntervieweeSearch',
        ...
        $actions['view'] = [
            ...
            'findModel' => static function ($id, \yii\rest\ViewAction $action) {
                ...
                $model = $query->where(['interviewee.id'=>$id])->one();
                if ($model === null) {
