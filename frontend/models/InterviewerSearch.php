<?php

namespace frontend\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use frontend\models\Interviewer;

/**
 * InterviewerSearch represents the model behind the search form of `frontend\models\Interviewer`.
 */
class InterviewerSearch extends Interviewer
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id'], 'integer'],
            [['name', 'bio'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Interviewer::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $dataProvider->sort->attributes['migrationName'] = [
            'asc' => ['migrationStatus.name' => SORT_ASC],
            'desc' => ['migrationStatus.name' => SORT_DESC],
        ];

        $dataProvider->sort->attributes['intervieweeName'] = [
            'asc' => ['interviewee.name' => SORT_ASC],
            'desc' => ['interviewee.name' => SORT_DESC],
        ];
        $dataProvider->sort->attributes['interviewerName'] = [
            'asc' => ['interviewer.name' => SORT_ASC],
            'desc' => ['interviewer.name' => SORT_DESC],
        ];


        $query->joinWith('interviews'); /*->joinWith('migration')->joinWith('gender')->joinWith('interviewee');*/


        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
        ]);

        $query->andFilterWhere(['like', 'name', $this->name])
            ->andFilterWhere(['like', 'bio', $this->bio]);

        return $dataProvider;
    }
}
